...
interfaces that actually have methods on it (that is why it has
...
I still don't have a good idea on how to get the _type pointers,
or how to manipulate the eface type. My guess would be to just cast
it as a pointer and do some
...
schedinit, at least according to go guru, is not called anywhere.
...
Sorry, got pretty far from the objective, lets go back to the
type system hacking fun. Lets start the copying fun, just like
the reflect package does, to inspect details on different types:
